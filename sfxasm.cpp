...
int main(int argc, char *argv[]) {
    ...
    printf("Nintendo SuperFX assembler 1.1 - (c) 2015 mohhingman\n");
    if (argc != 3) {
        printf("\nusage: sfxasm sourcefile outputfile\n");
        ...
        while (end_of_file == 0) {
            ...
            if (current_line[0] == ';' || current_line[0] == '\n') {
                    ...
                    if (current_line[i] == ' ') // at least 1 argument
                    {
                        ...
                        for (j = 0; j < maxargumentsize; j++) {
                            ...
                            if (j == (maxargumentsize - 1)) {
                                printf(
                                    "FATAL ERROR: Argument too long at Line %d",
                                    line_number);
                                fclose(fp);
                                fclose(of);
                                return 0;
                        ...
                        if (nomorearguments == 0) {
                            ...
                            for (j = 0; j < maxargumentsize; j++) {
                                ...
                                if (j == (maxargumentsize - 1)) {
                                    printf("FATAL ERROR: Argument too long at "
                                           "Line %d",
                                           line_number);
                                    fclose(fp);
                                    fclose(of);
                ...
                if (pass == 2) {
                    ...
                    if (arg_1_valid == 1 && arg_1[0] != '%' &&
                        arg_1[0] != '$' && arg_1[0] != '0' && arg_1[0] != '1' &&
                        arg_1[0] != '2' && arg_1[0] != '3' && arg_1[0] != '4' &&
                        arg_1[0] != '5' && arg_1[0] != '6' && arg_1[0] != '7' &&
                        arg_1[0] != '8' && arg_1[0] != '9') {
                        for (x = 0; x <= label_index; x++) {
                            if (strcmp(arg_1, label_names[x]) ==
                                0) { // found it!
                                arg_1_ispointer = 1;
                                // itoa(x, arg_1,10);
                                sprintf(arg_1, "%d", x);
                            ...
                            if (x == label_index) {
                                printf("FATAL ERROR. Reference was not found "
                                       "at line %i",
                                       line_number);
                                fclose(fp);
                                fclose(of);
                    ...
                    if (arg_2_valid == 1 && arg_2[0] != '%' &&
                        arg_2[0] != '$' && arg_2[0] != '0' && arg_2[0] != '1' &&
                        arg_2[0] != '2' && arg_2[0] != '3' && arg_2[0] != '4' &&
                        arg_2[0] != '5' && arg_2[0] != '6' && arg_2[0] != '7' &&
                        arg_2[0] != '8' && arg_2[0] != '9') {
                        for (x = 0; x <= label_index; x++) {
                            if (strcmp(arg_2, label_names[x]) ==
                                0) { // found it!
                                arg_2_ispointer = 1;
                                // itoa(x, arg_2,10);
                                sprintf(arg_2, "%d", x);
                ...
                for (x = 0; x < numberofinstructions * 3; x++) {
                    if (strcmp(current_instruction, sfx_instructions[x]) == 0) {
                        if (verbosemode == 1) {
                            ...
                            printf("Line %d,PC %d: '%s' type %s, hex '%s'\n",
                                   ...
                                   sfx_instructions[x], sfx_instructions[x + 1],
                                   sfx_instructions[x + 2]);
                        }
                    ...
                    if (x == ((numberofinstructions * 3) - 1)) {
                        printf("FATAL ERROR: Unrecognised Instruction %s at "
                               "Line %d",
                ...
                // 3 - COMPACT_DOUBLE - INSTRUCTION WITH 4-BIT ARGUMENT AND THEN
                // 16-BIT ARGUMENT 4 - DOUBLE-COMPACT - TWO COMPACT INSTRUCTIONS
                // SEQUENTIALLY 5 - COMPACT_SINGLE - INSTRUCTION WITH 4-BIT
                // ARGUMENT AND THEN 8-BIT ARGUMENT
                ...
                if (pass == 2) {
                    ...
                    case 0: // Single instruction with no arguments
                        ...
                        output_needed = 1;
                        snprintf(output_string, 5, "%02lX", outputcode);
                        break;
                    case 1: // BRANCH-wait for second pass - search for labels
                        ...
                        if (branch_temp > 255 || branch_temp < 0) {
                            printf("FATAL ERROR. Branch instruction out of "
                                   "range at line %i",
                                   line_number);
                            fclose(fp);
                            fclose(of);
                        ...
                        snprintf(output_string, 14, "0%lX", outputcode);
                        output_needed = 1;
                        break;
                    ...
                    case 2:
                        outputcode =
                            strtol(op_code, NULL,
                                   16); // Compact - Add to integer opcode.
                        if (arg_1_ispointer == 1) {
                        ...
                        output_needed = 1;
                        snprintf(output_string, 12, "%lX", outputcode);
                        break;
                    case 3: // Compact-double
                        ...
                        temp = temp * 256 *
                               256; // add compact instruction to outputcode
                        outputcode = outputcode + temp;
                        ...
                        outputcode = outputcode + temp;
                        output_needed = 1;
                        snprintf(output_string, 12, "%08lX", outputcode);
                        break;
                    case 4: // double compact
                        ...
                        outputcode = outputcode + temp;
                        output_needed = 1;
                        snprintf(output_string, 12, "%04lX", outputcode);
                        break;
                    case 5: // Compact-single
                        ...
                        temp =
                            temp * 256; // add compact instruction to outputcode
                        outputcode = outputcode + temp;
                        if (arg_2_ispointer == 1) {
                            temp = label_addresses[argconvert(arg_2)];
                        } else {
                        ...
                        outputcode = outputcode + temp;
                        output_needed = 1;
                        snprintf(output_string, 12, "%06lX", outputcode);
                        break;
                    }
                } else { // first pass
                    switch (op_type) {
